...
+ In the Spitzer R/G/B=24/8/4.5 image they show up with two different colours: red and lilac, which seem to alternate
  ...
  + If so, the grains would have to be really large
    + [ ] Look up papers on core-shine/cloud-shine
      ...
      + Better: Ysard:2018a Fig 13 shows that high albedo and high g @ 4.5 \mu{}m are achieved for grains > 1 \mu{}m size for various different compositions
...
#+BEGIN_SRC python :return figfile :results file
  ...
  for composition, ls in ["silicate", "-"], ["graphite", "--"]:
      ...
      for name in t.colnames[1:]:
          ax.plot(wav, t[name] + ts[name], ls=ls, lw=1, label=f"{composition} {name}")
          total += t[name] + ts[name]
          tot_abs += t[name]
          tot_sca += ts[name]
          tot_scg += ts[name]*tg[name]

  ax.plot(wav, total, '-', color='k', label='_none_')
...
+ Doing the same for our other objects
  ...
  + \sigma Ori has L_* = 6e4, R = 0.12 pc, L_IR = 15 Lsun
    + \tau = 2 L_IR/L_* = 5e-4
    + \eta = 245 \tau R_{pc} T_{4} / \kappa_{600} h_{1/4} L_{4} = 2.45e-3
...
+ Sensitivity to parameters
  + \tau = 2 L_IR/L_* and \eta =  245 \tau R_{pc} T_{4} / \kappa_{600} h_{1/4} L_{4}
...
+ Conversion to mass-loss rate
  ...
  + Mdot_-7 = 495 \tau R_{pc} T_{4} / \kappa_{600} h_{1/4} V_{3}
