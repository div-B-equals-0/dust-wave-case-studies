...
#+BEGIN_SRC python :return figfile :results file
  ...
  mod03 = CloudyModel(os.path.join(CDC, "models", "shell-R001-n23-LP_Ori20BB"))
  ...
  ax.fill_between(wav, star_sed, facecolor="yellow", alpha=0.3)
  ax.fill_between(wav, shell_sed, facecolor="red", alpha=0.3)
...
+ [ ] We will compare with the SED from a Cloudy model
  ...
  + Other options:
    1. [X] Cloudy - I can run a special model with the star extincted by a certain column
       - This has to be done by running an actual model out to AV = 1.5, which we wwill try
    2. Look at getting Draine's extinction curve from somewhere
+ [ ] And I also want to put on some magnitudes from the literature
