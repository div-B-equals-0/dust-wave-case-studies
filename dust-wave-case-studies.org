...
+ Flux conversions:
  ...
  + And source 67
    ...
    + (0 + 0.775 + 21.2982 + 1.056 + 0.864) = 23.99e-10 = 2.4e-10
    + This is very different from the 6e-9 in table! That seemed to be based on a very high-T fit, which cannot be right.
  + So, in both cases, over 90% of total IR flux comes from 8, 24, 60 micron bands
...
+ KE passing through = \pi R_{0}^{2} v/2 P_{stream}
+ Radiant energy absorbed = 0.5 L (1 - e^{-\tau})
